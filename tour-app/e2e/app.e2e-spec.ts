...
describe('Starting tests for tour-app', function() {
  ...
  it('website title should be tour-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('tour-app');
    })
  });
  ...
  it('navbar-brand should be tour-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tour-app');
    });
  });
